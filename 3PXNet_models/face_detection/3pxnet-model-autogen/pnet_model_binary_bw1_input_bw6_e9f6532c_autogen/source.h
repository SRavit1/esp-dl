...
/*int main(){ 
	...
	for(int img = 0; img < 1; img++) {
		...
		int res = CnXnorNoBinWrap(l4act_bin, l4wght, C4Z, C4XY, C4XY, C4Z, C4KXY, C4KXY, C4KZ, output, C4PD, C4PL, bn1mean, bn1var, bn1gamma, bn1beta);
		int res = CnXnorNoBinWrap(l5act_bin, l5wght, C5Z, C5XY, C5XY, C5Z, C5KXY, C5KXY, C5KZ, output, C5PD, C5PL, bn1mean, bn1var, bn1gamma, bn1beta);
		float max = -INFINITY; 
		int maxIdx = 0; 
		for (int i = 0; i <10; i++) { 
			 printf("%f, ", output[i]);
