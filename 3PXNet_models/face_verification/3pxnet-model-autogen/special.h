 ...
 */
void refCn_bwn(int16_t * pAct, int16_t * pKrn, const uint16_t dpth, const uint16_t wdth, const uint16_t hght, const uint16_t kwdt, const uint16_t khgt, const uint16_t knum, float * pOut,
   ...
   for (uint16_t y = 0; y < ((hght-khgt+1)/pool); y++) {
      ...
      for (uint16_t x = 0; x < ((wdth-kwdt+1)/pool); x++) {
         ...
         for (uint16_t kn = 0; kn < knum; kn++) {
                  ...
                  for (uint16_t ky = 0; ky < khgt; ky++) {
                     ...
                     for (uint16_t kx = 0; kx < kwdt; kx++) {
                        ...
                        for (uint16_t kz = 0; kz < dpth; kz++) {
                           //printf("Attempting to access values at locations %p %p\n", pIn, pWgt);
                           int16_t value = *pIn++;
            ...
            if (pMean) {
               maxTemp = ((*pGm++)*(((float)maxTemp - *pMn++)/ *pSg++) + *pBt++);
            }
            *pOut++ = maxTemp;
         } // Kernel
      } // X
