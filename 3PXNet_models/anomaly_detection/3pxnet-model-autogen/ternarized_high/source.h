...
int main(){ 
	...
	for(int img = 0; img < 1; img++) {
		...
		res = Fc3pxnWrap(l1act_bin, l1wght, l1ind, F1NPI, F1O, l2act_bin, bn1thr, bn1sign, bn1offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc1 res is 1");
		int64_t fc1_time = esp_timer_get_time();
		res = Fc3pxnWrap(l2act_bin, l2wght, l2ind, F2NPI, F2O, l3act_bin, bn2thr, bn2sign, bn2offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc2 res is 1");
		int64_t fc2_time = esp_timer_get_time();
		/*
		res = Fc3pxnWrap(l3act_bin, l3wght, l3ind, F3NPI, F3O, l4act_bin, bn3thr, bn3sign, bn3offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc3 res is 1");
		int64_t fc3_time = esp_timer_get_time();
		res = Fc3pxnWrap(l4act_bin, l4wght, l4ind, F4NPI, F4O, l5act_bin, bn4thr, bn4sign, bn4offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc4 res is 1");
		int64_t fc4_time = esp_timer_get_time();
		res = Fc3pxnWrap(l5act_bin, l5wght, l5ind, F5NPI, F5O, l6act_bin, bn5thr, bn5sign, bn5offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc5 res is 1");
		int64_t fc5_time = esp_timer_get_time();
		res = Fc3pxnWrap(l6act_bin, l6wght, l6ind, F6NPI, F6O, l7act_bin, bn6thr, bn6sign, bn6offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc6 res is 1");
		int64_t fc6_time = esp_timer_get_time();
		res = Fc3pxnWrap(l7act_bin, l7wght, l7ind, F7NPI, F7O, l8act_bin, bn7thr, bn7sign, bn7offset, 1, 1);
		if (res) ESP_LOGI(TAG, "ERROR: fc7 res is 1");
		int64_t fc7_time = esp_timer_get_time();
		res = Fc3pxnWrap(l8act_bin, l8wght, l8ind, F8NPI, F8O, l9act_bin, bn8thr, bn8sign, bn8offset, 1, 1);
		...
		res = Fc3pxnNoBinWrap(l10act_bin, l10wght, l10ind, F10NPI, F10O, output, bn10mean, bn10var, bn10gamma, bn10beta, 1, 1);
